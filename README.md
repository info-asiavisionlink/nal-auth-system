...
| `ALLOWED_TOOL_ORIGINS` | 外部ツールのオリジン（CORS・カンマ区切り） |
| `NEXT_PUBLIC_APP_URL` | ダッシュボード公開 URL（任意） |
...
## ツール起動・クレジット API（外部ツール連携）

| 用途 | データ源 |
|------|----------|
| ダッシュボード表示 | Google Sheets（`tool_id` = `tool_key`） |
| クレジット消費・残高 | Supabase `tools` / `profiles` |

**`credit_cost` をツール側から送らないでください**（送っても無視）。  
実消費はサーバーが `tools.credit_cost` のみ使用します。

### Supabase SQL（必須）

1. **`supabase/credits-schema.sql`**（未実行なら）
2. **`supabase/tool-access-tokens.sql`**（一時トークン + `consume_credit_for_user` RPC）

### ツール起動（全ツール共通）

`POST /api/tools/open`（ダッシュボードから Cookie 認証）
...
{
  "success": true,
  "url": "https://your-tool.vercel.app?access_token=xxxx"
}
```

ダッシュボードの「ツールを開く」はこの API を呼び、返却 URL へ遷移します（**消費しない**）。

### トークン検証（外部ツール初回）

`GET /api/tools/token/verify`  
...
const TOOL_KEY = "google_map_leads";
const STORAGE_KEY = "nal_tool_access_token";

// 1. URL から token を保存し URL をクリーン
...
// 2. 残高・ツール情報
const verifyRes = await fetch(`${DASHBOARD_BASE_URL}/api/tools/token/verify`, {
  headers: { Authorization: `Bearer ${accessToken}` },
});
const verify = await verifyRes.json();

// 3. 処理成功後に消費（credit_cost は送らない）
const consumeRes = await fetch(`${DASHBOARD_BASE_URL}/api/credits/consume`, {
  method: "POST",
  headers: authHeaders,
  body: JSON.stringify({
    tool_key: TOOL_KEY,
...
環境変数 **`ALLOWED_TOOL_ORIGINS`** にツールのオリジンをカンマ区切りで登録してください（CORS）。
